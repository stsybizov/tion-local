...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback):
    ...
    # HA dedups entities by unique_id, so re-adding on reload is safe (and required — a
    # module-level "already configured" guard would skip recreation on reload, orphaning
    # the climate entity).
...
class TionClimateEntity(ClimateEntity, CoordinatorEntity):
    ...
    _attr_min_temp = 0
    _attr_max_temp = 25  # Tion 4S heats incoming air up to 25 °C
    ...
    _attr_translation_key = "tion"  # localizes preset_mode states (translations/*.json climate.tion)
    _attr_has_entity_name = True    # use the device name ("Tion 4S"), no separate climate name
    _attr_name = None
    ...
    def __init__(self, hass: HomeAssistant, instance: TionInstance):
        ...
        # Build a fresh per-instance list so we never mutate the shared class-level
        # list (which would accumulate duplicate PRESET_AWAY across instances/reloads).
    ...
    async def async_set_preset_mode(self, preset_mode: str):
        ...
        if preset_mode == PRESET_SLEEP and self.preset_mode != PRESET_SLEEP:
            ...
            if self._saved_fan_mode is None:
                self._saved_fan_mode = self._safe_fan()
            actions.append([self.async_set_fan_mode, {'fan_mode': min(self._safe_fan(), self.sleep_max_fan_mode)}])
    ...
    def _safe_fan(self, default: int = 1) -> int:
        """Current fan speed as int, tolerant of the breezer being off (fan_mode None)."""
        try:
    ...
    def _handle_coordinator_update(self) -> None:
        self._get_current_state()
        if self._safe_fan() != self.boost_fan_mode and (self._is_boost or self.preset_mode == PRESET_BOOST):
    ...
    def _get_current_state(self):
        ...
        self._attr_extra_state_attributes = {
            ...
            'state_raw': self.coordinator.data.get("state_raw"),
        }
        is_on = self.coordinator.data.get("is_on")
        heater = self.coordinator.data.get("heater")
        self._attr_hvac_mode = HVACMode.OFF if not is_on else \
            HVACMode.HEAT if heater else HVACMode.FAN_ONLY
        # Don't report a running hvac_action: HEATING/FAN translate to the same word as the
        # HEAT/FAN_ONLY mode ("Обогрев", "Вентиляция"), so cards that show "{action} ({mode})"
        # render a redundant "Обогрев (Обогрев)" / "Вентиляция (Вентиляция)". The mode already
        # conveys the state; heater_power shows whether the element is actually on.
        self._attr_hvac_action = HVACAction.OFF if not is_on else None
